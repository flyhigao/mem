...
- 特性：
  ...
  - 自动保留各用户最新 500 条文本记录
  - 小文件批量中转：支持多文件上传，单文件 ≤ 10MB，总存储上限 1GB 自动循环清理最旧文件
  - 支持自定义监听地址（`-addr 127.0.0.1:48081`），配合 Nginx 反代最佳
  - 网页控制台：快捷输入发送、历史列表倒序展示、一键复制、单条删除、文件列表与一键下载、定时自动刷新
...
- 功能：
  ...
  - 主界面消息历史查看与一键复制
  - 小文件中转：多文件批量选择上传（单文件 ≤ 10MB），文件列表浏览与直链下载

---

## 未来规划 (Roadmap)

关于低功耗长连接推送、夜间飞行模式（仅 Wi-Fi）与 Android 深睡（Deep Doze）穿透唤醒的完整技术演进方案已落盘归档，详见：
