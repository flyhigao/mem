...
> 📌 本文档为通用安装说明；**本机直连 + Nginx 容器反代**的完整生产部署方案（含可复制的配置、验证与回滚步骤）见 [`DEPLOY.md`](DEPLOY.md)。
...
| `-upload-dir` | `UPLOAD_DIR` | `uploads` | 上传文件存储目录 |

### 存储限额与规则
- **单文件上限**：≤ 10MB
- **历史总容量上限**：每用户 ≤ 1GB
- **自动循环淘汰**：当总存储超过 1GB 时，系统自动删除最旧文件腾出空间 (FIFO)
